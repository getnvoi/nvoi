...
internal/api/
  ...
  handlers/
    router.go            Gin routes + Swagger UI mount (/swagger/*any)
    ...
    swagger_types.go     Named request/response types for swagger annotations

  docs/                  Generated by swag — do not edit manually
    docs.go              Embedded swagger spec (imported by router.go)
    swagger.json         Swagger 2.0 spec (JSON)
    swagger.yaml         Swagger 2.0 spec (YAML)
...
Interactive docs at `/swagger/index.html` when the API is running. Powered by `swaggo/swag` (Swagger 2.0).

**How it works:**
1. Handler comments use `@Summary`, `@Param`, `@Success`, `@Failure`, `@Router` annotations
2. `bin/swag` generates `internal/api/docs/` (docs.go + swagger.json + swagger.yaml)
3. `router.go` imports the generated docs and mounts Swagger UI via `gin-swagger`
...
- Same package (handlers): use type name directly (`errorResponse`, `loginRequest`)
- `internal/api` types: use `api.User`, `api.Workspace`, `api.Deployment`, etc.
- `pkg/core` types: use fully qualified `github_com_getnvoi_nvoi_pkg_core.DescribeResult` (required because `internal/core` has the same package name `core`)
- `pkg/provider` types: use `github_com_getnvoi_nvoi_pkg_provider.ResourceGroup` (same reason)

**Request/response types:**
- Inline `gin.H{}` maps → not swagger-documented (only used for error envelopes)
- Named types in `swagger_types.go` → used for request bodies and structured responses
- Model types in `models.go` → `api.User`, `api.Workspace`, `api.Repo`, `api.Deployment`, etc.
- External types → `pkg/core.DescribeResult`, `pkg/provider.ResourceGroup` (referenced with json tags)

## API routes

```
GET    /swagger/*any                                   Swagger UI (interactive docs)
