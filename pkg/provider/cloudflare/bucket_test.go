...
func testBucketClient(t *testing.T, handler http.Handler) *BucketClient {
	ts := httptest.NewServer(handler)
	t.Cleanup(ts.Close)
	c := NewBucket(map[string]string{"api_key": "test-key", "account_id": "acct123"})
...
func TestValidateCredentials_EmptyKey(t *testing.T) {
	c := NewBucket(map[string]string{"api_key": "", "account_id": "acct123"})
...
func TestValidateCredentials_EmptyAccountID(t *testing.T) {
	c := NewBucket(map[string]string{"api_key": "test-key", "account_id": ""})
	err := c.ValidateCredentials(context.Background())
	if err == nil {
