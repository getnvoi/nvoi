...
func (d *DNSClient) ensureCNAME(ctx context.Context, domain, target string) error {
	...
	for _, rec := range existing {
		...
		return d.updateRecord(ctx, rec.ID, cfDNSRecord{Type: "CNAME", Name: fqdn, Content: target, TTL: 300})
	}
	return d.createRecord(ctx, cfDNSRecord{Type: "CNAME", Name: fqdn, Content: target, TTL: 300})
}
