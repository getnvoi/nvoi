...
func (c *Client) Bootstrap(ctx context.Context, dc *provider.BootstrapContext) (*kube.Client, error) {
	...
	// Builders are out-of-band: ProvisionBuilders runs separately on the
	// operator's machine; Bootstrap here only touches the k8s cluster.
...
func (c *Client) TeardownOrphans(ctx context.Context, dc *provider.BootstrapContext) error {
	...
	// Include per-builder cache volumes in the desired set so orphan sweep
	// doesn't nuke the buildkit cache of a declared builder.
...
func (c *Client) Teardown(ctx context.Context, dc *provider.BootstrapContext, deleteVolumes bool) error {
	...
	// Builders first (no k8s drain needed), then workers, then master.
...
// ProvisionBuilders converges role: builder EC2 instances + their per-builder
// cache EBS volumes + the shared builder security group. Idempotent — same
// shape as Hetzner's implementation. Runs BEFORE Bootstrap on the SSH-build
// dispatch path (CLI calls this first, then BuilderTargets, then dispatches).
// No-op when no role: builder servers are declared.
...
// provisionBuilder is AWS's per-builder orchestration. Distinct from
// provisionServer — builders are NOT k8s nodes. Sequence mirrors Hetzner:
// EnsureServer with builder cloud-init + BuilderFirewall → SSH wait → swap →
// cache volume create+mount → enable docker + binfmt post-mount.
...
// BuilderTargets returns every role: builder EC2 instance this provider
// manages for the current cluster. Lookup by tag. Empty slice when none —
// callers feature-gate on len() == 0.
func (c *Client) BuilderTargets(ctx context.Context, dc *provider.BootstrapContext) ([]provider.BuilderTarget, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("aws.BuilderTargets: %w", err)
	}
...
// splitServers: three-way split (masters / workers / builders). Builders are
// NEVER iterated by Bootstrap — they are provisioned out-of-band by
// ProvisionBuilders before the deploy is dispatched into a builder.
// See pkg/provider/CLAUDE.md for the dispatch ordering.
